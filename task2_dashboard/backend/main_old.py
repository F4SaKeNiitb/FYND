...
async def submit_review(submission: ReviewSubmission):
    ...
    """
    try:
        # OPTIMIZATION: Generate user response immediately, process admin data in background
        # This reduces user wait time significantly
        
        # Quick sentiment analysis for user response
        sentiment = "positive" if submission.rating >= 4 else "negative" if submission.rating <= 2 else "neutral"
        
        # Generate only user response (fast path for user)
        user_response = await generate_user_response(submission.rating, submission.review, sentiment)
        
        # Create review object with placeholder admin data
        review_id = f"review_{int(datetime.now().timestamp() * 1000)}_{os.urandom(4).hex()}"
        
        # Quick fallback summary and actions (no AI call)
        quick_summary = f"{sentiment.capitalize()} {submission.rating}-star review: {submission.review[:80]}..."
        quick_actions = get_fallback_actions(sentiment)
        ...
        new_review = {
            ...
            "recommendedActions": quick_actions,
        }
        
        # Save immediately so user gets fast response
        reviews = read_reviews()
        reviews.append(new_review)
        write_reviews(reviews)
        
        # Schedule background processing for admin data (non-blocking)
        asyncio.create_task(process_admin_data(review_id, submission.rating, submission.review))
...
def get_fallback_actions(sentiment: str) -> List[str]:
    """Get quick fallback actions without AI"""
    if sentiment == "positive":
        return ["Continue delivering excellent service", "Share feedback with team"]
    elif sentiment == "negative":
        return ["Follow up with customer", "Review processes for improvement"]
    return ["Review feedback for insights", "Monitor for patterns"]


async def process_admin_data(review_id: str, rating: int, review_text: str):
    """Background task to process admin data with AI"""
    try:
        # Full sentiment analysis
        sentiment = await analyze_sentiment(review_text)
        
        # Generate admin summary and actions
        admin_summary, recommended_actions = await asyncio.gather(
            generate_admin_summary(rating, review_text, sentiment),
            generate_recommended_actions(rating, review_text, sentiment)
        )
        
        # Update the review with AI-generated admin data
        reviews = read_reviews()
        for r in reviews:
            if r.get('id') == review_id:
                r['aiSummary'] = admin_summary
                r['recommendedActions'] = recommended_actions
                r['sentiment'] = sentiment
                break
        write_reviews(reviews)
        print(f"[DEBUG] Background processing complete for {review_id}")
    except Exception as e:
        print(f"[ERROR] Background processing failed for {review_id}: {e}")
